...
object Watermark{
     ...
     */
     ...
     // {"CreatedTime":"2019-02-05 10:40:00", "Type":"BUY", "Amount": 500, "BrokerCode": "ABC"}
     ...
     val windowedDF = newDF
     .withWatermark("createdAt","30 minute")
     .groupBy(window(col("createdAt"),"15 minute")).agg(sum("Buy").alias("TotalBuy"), sum("Sell").alias("TotalSell"))


     // Window state is more than 30 mns it will clear
     // no records will be entertained after winwo time is crossed
     val streamingQuery = windowedDF.writeStream
     .format("console")
     .queryName("Write Stream of User Data")
     .outputMode("update")
     .option("path","F:\\SparkLearning\\output")
     .option("checkpointLocation","F:\\SparkLearning\\checkpointWatermark")
